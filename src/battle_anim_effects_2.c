...
static void AnimAirWaveProjectile_Step1(struct Sprite *sprite)
{
    ...
    if (1 & task->data[7])
        sprite->x2 = -Q_8_8_TO_INT((u16)sprite->data[1]);
    else
        sprite->x2 = Q_8_8_TO_INT((u16)sprite->data[1]);;

    if (1 & task->data[8])
        sprite->y2 = -Q_8_8_TO_INT((u16)sprite->data[2]);
    else
        sprite->y2 = Q_8_8_TO_INT((u16)sprite->data[2]);
...
void AnimAirWaveProjectile(struct Sprite *sprite)
{
    ...
    if (sprite->data[0]-- <= 0)
    {
        sprite->data[0] = 8;
        task->data[5] = 4;
        a = Q_8_8_inv(Q_8_8(16));
        sprite->x += sprite->x2;
        sprite->y += sprite->y2;
        sprite->y2 = 0;
        sprite->x2 = 0;
        if (task->data[11] >= sprite->x)
            b = Q_8_8((u8)(task->data[11] - sprite->x));
        else
            b = Q_8_8((u8)(sprite->x - task->data[11]));

        if (task->data[12] >= sprite->y)
            c = Q_8_8((u8)(task->data[12] - sprite->y));
        else
            c = Q_8_8((u8)(sprite->y - task->data[12]));

        sprite->data[2] = 0;
        sprite->data[1] = 0;
        sprite->data[6] = 0;
        sprite->data[5] = 0;
        sprite->data[3] = Q_8_8_mul(Q_8_8_mul(b, a), Q_8_8_inv(Q_8_8(1.75)));
        sprite->data[4] = Q_8_8_mul(Q_8_8_mul(c, a), Q_8_8_inv(Q_8_8(1.75)));
...
void AnimTask_AirCutterProjectile(u8 taskId)
{
    ...
    gTasks[taskId].data[6] = Q_8_8_mul(gTasks[taskId].data[5], Q_8_8(0.5));
